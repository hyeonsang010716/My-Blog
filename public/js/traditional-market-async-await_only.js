async function drawMap() {
  ...
  for(let i = 0; i < locations.length; i++) { 
    ...
    var infowindow = new kakao.maps.InfoWindow( {
      content : `<div style="width:250px;padding:3px;">${locations[i].title}(${locations[i].openday})<br>${locations[i].addr}<br>경도:${locations[i].x}<br>위도:${locations[i].y}</div>`   // 인포윈도우에 표시할 내용
    });
...
async function getLocations(markets) {
  ...
  for(let i=0; i < markets.length; i++) {
    waitingList[i] = await getLocation(markets[i], locations);
  }
  return locations;
}
...
async function getLocation(market, locations) {
  ...
  const response = await fetch(`https://dapi.kakao.com/v2/local/search/address.json?analyze_type=similar&page=1&size=10&query=${query}`, {
    headers:  {
                "Authorization": "KakaoAK 526241a055f71dd9883a2b285337330a"
              }
  });
  ...
  if(location && location.address_name && location.x && location.y) {
    locations.push({
      addr : location.address.address_name,
      x : location.x,
      y : location.y,
      title: market.title,
